...
const SYSTEM_NOISE = [
  ...
  /^Службу відключено/i,
  /^цю (беремо|візьмемо|беруть)/i, // "Цю беремо теж? 🥹"
  /^тільки (цю|цього|їх|його|її) (додати|внести)/i, // "тільки цю жіночку додати"
  /^також внесіть/i,
  /^внесіть/i,
  ...
  /^підкажіть,? (чи|будь)/i,
  /^чи все ж/i,
  /^доброго (ранку|дня|вечора)[\s),.!]*$/i,
  /^добрий (ранок|день|вечір)[\s),.!]*$/i,
  // Анкеты кандыдатаў украінскія:
  /^[А-ЯЁІЎ]{2,}\s+[А-ЯЁІЎ]{2,}\s+[A-Z]{1,2}\d{5,}/u, // TARUTINA MARYNA FZ123
  /^\+\d{10,13}[\s\n]+[a-z0-9._%+-]+@/i, // тэл + email
  /^так так,/i, // "так так, то я переслала..."
...
const RECRUITER_CHAT_NOISE = [
  ...
  /^[A-Z]+\s+[A-Z]+\s+[A-Z0-9]{6,10}/, // Анкеты тыпу SALINA VIKTORIIA GNO78732
  /^\+\d{10,13}\s+\+\d{10,13}/, // Проста спіс тэлефонаў
];
